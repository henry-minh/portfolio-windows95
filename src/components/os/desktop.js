...
function Desktop() {
    ...
    // useState to keep track of most recently opened browser
    const [z, setZ] = useState(1);
    //useState for Clicking the Start Button
    const [start, setStart] = useState(false);
    //useState for login page
    const [logIn, setLogIn] = useState(true);
    // Application Library Data
    ...
    const addBrowser = useCallback(
        (browserTitle) => () => {
            ...
            setItems(
                items.map((item) =>
                    item.browserName === browserTitle
                        ? { ...item, hidden: false, state: true, zPosition: z, taskBarClassName: "taskbar-selected" }
                        : { ...item, taskBarClassName: "taskbar-unselected" }
                )
            );
            console.log(items);
        },
    ...
    const changeBrowserFocus = useCallback(
        (browserTitle) => () => {
            ...
            setItems(
                items.map((item) =>
                    item.browserName === browserTitle
                        ? { ...item, hidden: false, zPosition: z, taskBarClassName: "taskbar-selected" }
                        : { ...item, taskBarClassName: "taskbar-unselected" }
                )
            );
            console.log(items);
    ...
    const signInFunction = useCallback(() => {
        setLogIn(false);
    }, [logIn]);
    return (
        <div className="Desktop">
            ...
            <div id="desktop-shortcut-container" draggable>
                <button id="desktop-shortcut-button" onClick={addBrowser("About Me")}>
                    <img src="./ComputerIcon.ico" id="desktop-button-image"></img>
            ...
            <div id="desktop-shortcut-container" onClick={addBrowser("Open Source")}>
                <button id="desktop-shortcut-button">
                    <img src="./FolderIcon.ico" id="desktop-button-image"></img>
                    <p id="desktop-shortcut-text">Open Source</p>
                </button>
            </div>
            <div id="desktop-shortcut-container">
                <button id="desktop-shortcut-button" onClick={addBrowser("Photos")}>
                    <img src="./PictureIcon.ico" id="desktop-button-image"></img>
            ...
            <div id="desktop-shortcut-container">
                <button
                    id="desktop-shortcut-button"
                    onClick={() => {
                        window.open("https://capable-pudding-12bab4.netlify.app/", "_blank");
                    }}
                >
                    <img src="./Browser.ico" id="desktop-button-image"></img>
                    <p id="desktop-shortcut-text">2D Web Portfolio Version</p>
                </button>
            </div>
*/}
                    ...
                    (item) =>
                        item.state == true && (
                            <TaskBarButtons
                                ...
                                item={item.browserName}
                                icon={item.icon}
                                taskbarClassName={item.taskBarClassName}
                                clickTaskbarIcon={changeBrowserFocus}
                            ></TaskBarButtons>
                        )
                )}
